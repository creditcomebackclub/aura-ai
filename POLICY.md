...
|---|---|---|
| `read` | No state change. Always autonomous. | `list_database_tables`, `get_table_schema`, `query_database_table`, `count_database_rows`, `get_outstanding_balances`, `calculate_financial_metrics`, `get_client_snapshot`, `get_client_current_phase`, `check_email`, `check_calendar`, `get_goals`, `get_goal_plans`, `query_finances`, `check_blackboard`, `search_web`, `list_deletable_test_letters`, `list_skills`, `view_skill` |
| `reversible_write` | Changes state non-destructively, either from a direct owner command or by only *staging* a later destructive step. | `add_goal`, `set_goal_plan`, `update_goal_step`, `update_goal_status`, `log_finance`, `save_semantic_memory`, `manage_skill`, `create_calendar_event`, `propose_test_letter_deletion` |
...
Goal-plan writes are internal organization only. A stored step such as “email the
proposal” is inert task text and never satisfies the current-turn authorization
required by an email, calendar, messaging, purchase, or destructive tool.
