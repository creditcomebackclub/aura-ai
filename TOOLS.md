...
| Voice transcription | `AURA_STT_PROVIDER` (`deepgram` when `DEEPGRAM_API_KEY` is set, else `openai`) + Deepgram (`DEEPGRAM_API_KEY`, `AURA_DEEPGRAM_MODEL` default `nova-3`) or OpenAI (`OPENAI_API_KEY` + `AURA_TRANSCRIBE_MODEL` default `gpt-4o-mini-transcribe`; falls back to `whisper-1`). Deepgram failures fall back to OpenAI when keyed. | Throws when the active provider lacks a key (or both fail). |
