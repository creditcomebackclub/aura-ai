...
| `aura_state` | `owner_id`, `key`, `value` jsonb, `updated_at` | Primary key `(owner_id, key)`. Known keys: `owner_profile_v1` (pinned owner profile, entries keyed e.g. `people.taylor`, `communication.generic_signoff`, `pronunciation.<term>`, each `{kind, value, pinned, source, confidence, instruction}`), `executive_loop_v1` (mailbox identity, inbox/sent cursors, calendar fingerprints, meeting briefs), `web_search_daily_usage`, `blackboard_digest_date`, `blackboard_error`, `overdue_clients`, `financial_metrics` |
