...
| Executive Loop | `AURA_EXECUTIVE_LOOP` (default true), direct email OAuth for inbox monitoring, Google Calendar OAuth for structured events; optional `AURA_EXECUTIVE_QUIET_START`/`END` (default `21`/`7`) and `AURA_MEETING_BRIEF_MIN_MINUTES`/`MAX_MINUTES` (default `8`/`20`) | Each unavailable provider degrades independently; durable task reminders still run |
